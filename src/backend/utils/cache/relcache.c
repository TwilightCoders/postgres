...
RelationGetIndexAttrBitmap(Relation relation, IndexAttrBitmapKind attrKind)
{
	...
	/*
	 * ProgreSQL: a leaf partition with a spanning (GLOBAL) index on an ancestor
	 * root owns no local index but still has a non-empty hot-blocking set (the
	 * spanning key columns -- see progresql_add_spanning_hotblocking_attrs).
	 * Such a leaf must NOT take the "no local indexes" fast-path exits below, or
	 * a spanning-key UPDATE would be wrongly treated as HOT.
	 */
	spanning_leaf = progresql_leaf_has_spanning_ancestor(relation);

	/* Fast path if definitely no indexes */
	if (!RelationGetForm(relation)->relhasindex && !spanning_leaf)
		return NULL;
...
restart:
	...
	if (indexoidlist == NIL && !spanning_leaf)
		return NULL;
	...
	/*
	 * ProgreSQL: a leaf partition's spanning (GLOBAL) index lives on an
	 * ancestor root and is not in indexoidlist above, but its key columns must
	 * still block HOT updates on this leaf (see the helper's comment).  Gated on
	 * spanning_leaf so non-spanning partitions pay only the single detector walk.
	 */
	if (spanning_leaf)
		progresql_add_spanning_hotblocking_attrs(relation, &hotblockingattrs);
