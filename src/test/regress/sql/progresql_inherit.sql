...
-- Section 9b: a grandchild leaf added to the multi-level tree AFTER the spanning
-- index exists must register its partseq via the ancestor walk -- its direct
-- parent mld_us carries no spanning index of its own, so the partition
-- index-clone path (which only inspects the direct parent) misses it.  Before
-- the fix the new leaf got no partseq and failed closed ("no partseq for
-- partition") at the first INSERT.
CREATE TABLE mld_us_26 PARTITION OF mld_us
    FOR VALUES FROM ('2026-01-01') TO ('2027-01-01');
INSERT INTO mld VALUES (3, 'us', '2026-06-01');  -- ok: new grandchild leaf is registered + usable
INSERT INTO mld VALUES (1, 'us', '2026-07-01');  -- ERROR: dup id=1 enforced on the new grandchild
SELECT id, count(*) FROM mld GROUP BY id HAVING count(*) > 1;                     -- (none)
-- DROP of the after-index grandchild cleans its partseq map (OID-reuse safe)
DROP TABLE mld_us_26;
INSERT INTO mld VALUES (1, 'us', '2025-06-15');  -- ERROR: still enforced via the build-first leaf
SELECT id, count(*) FROM mld GROUP BY id HAVING count(*) > 1;                     -- (none)
