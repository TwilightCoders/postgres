...
| Function | Returns |
|---|---|
| `progresql_version()` | the fork **release** version (`'0.2.3'`, matching the `v18.3-X.Y.Z` tag), distinct from the PostgreSQL base reported by `server_version` — the supported fork-detection + version-gate hook (stock PostgreSQL has no such function) |
